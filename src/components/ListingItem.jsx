...
export default function ListingItem({ listing, id, onDelete, onEdit }) {
  return <li className="relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]">
   <Link className="content" to={`/category/${listing.type}/${id}`}>
      ...
      />
      <Moment className="absolute top-2 left-2 bg-[#3377cc] text-white uppercase text-sm font-semibold rounded-md px-2 py-1 shadow-lg" fromNow>{listing.timestamp?.toDate()}
      </Moment>
      <div className="w-full p-[10px]">
        ...
        <p className="font-semibold ml-4 text-xl truncate">
          {listing.name}
        </p>
        <p className="text-[#457b9d] mt-2 ml-4 font-semibold">
        ₦
          {listing.offer 
        ...
        <div className="flex items-center mt-[10px] space-x-3">
          ...
          <div className="flex items-center space-x-1">
             <p 
               className="font-bold text-xs ml-4">{listing.bathrooms > 1 ? `${listing.bathrooms} Baths` : "1 Bath"}
              </p>
          </div>
        </div>
      </div>
