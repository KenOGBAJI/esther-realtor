  ...
  useEffect(() =>{
    async function getRealtor() {
      ...
      if(docSnap.exists) {
        ...
        toast.error("Could not get Realtor data");
      }
    }
    getRealtor();
  }, [userRef])
  ...
  return (
    <>{realtor !== null &&  (
      <div className='flex flex-col w-full'>
        <p classname="">Contact {realtor.name} for the {listing.name.toLowerCase()}
        </p>
        ...
        <a href={`mailto:${realtor.email}?Subject=${listing.name}&body=${message}`}>
          <button className='px-7 py-3 bg-blue-600 text-white rounded text-sm uppercase shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out w-full text-center mb-6' type='button'>Send Message
          </button>
        </a>
      </div>
    )}</>
